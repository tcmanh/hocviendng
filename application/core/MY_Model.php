...
class MY_Model extends Base_Model {
	...
	function upload_image($images){
        ...
        if($this->upload->do_upload($images)){
            ...
            $resize_conf = array(
                'source_image'  => $check['full_path'], 
                'new_image'     => 'assets/uploads/member_thumbs/'.$check['file_name'],
                'width'         => 150,
                'height'        => 100
                );
        ...
        $images_name = $check['file_name']; 
        return $images_name;
    }

    function upload_multi_file($image, $folder){
        $name_array = array();
        ...
        for($i=0; $i<=$count-1; $i++) {
        	if($file['size'][$i] > 0){
	            ...
	            $format = substr($file['type'][$i], 0, 5);
	            $_FILES['userfile']['tmp_name'] = $file['tmp_name'][$i];
	            $_FILES['userfile']['error']    = $file['error'][$i];
	            $_FILES['userfile']['size']     = $file['size'][$i];    
	            $this->load->library('upload', $config);
	            $this->upload->initialize($config);
	            if($file['name'][$i] != '' && $format == 'image'){
	                if($this->upload->do_upload()){
	                    $images_name[] = $this->upload->data();      
	                }
	  	            else{
	                    $images_name = FALSE;
	                } 
