...
class MY_Controller extends MX_Controller {
	...
	private function _setup()
	{
		...
		$this->mTitlePrefix = empty($config['page_title_prefix']) ? '' : $config['page_title_prefix'];
		$this->mTitle = empty($config['page_title']) ? '' : $config['page_title'];
		...
		if ( !empty($this->mPageAuth[$uri]) && !$this->check_in_array($this->mPageAuth[$uri], get_current_users()->users_groups ) )
		{
		...
		if ( $this->ion_auth->logged_in() )
		{
			...
			//$url_user = "https://quanly.seoulspa.vn//api/users/get_user?id=".get_current_user_id();
			//$this->mUser = call_curl($url_user);
			$this->mUser = get_current_users();
			if ( !empty($this->mUser) )
			{
				//$this->mUserGroups = $this->ion_auth->get_users_groups($this->mUser->id)->result();
				$this->mUserGroups = get_current_user_groups();
				//echo $this->config('adminlte');
				// TODO: get group with most permissions (instead of getting first group)
				$this->mUserMainGroup = $this->mUserGroups[0]->description;
				//$this->mUserMainGroup = 'skin-blue';	
			}
		}
	...
	protected function render($view_file, $layout = 'default')
	{
		...
		$this->mViewData['page_title'] = $this->mTitlePrefix.$this->mTitle;
		...
		//$url_user = "http://quanly.seoulspa.vn//api/users/get_user?id=".get_current_user_id();
		//$this->mViewData['user'] = call_curl($url);
	...
	protected function check_in_array($array1, $array2)
	{
		foreach ($array1 as $value1) {
			foreach ($array2 as $value2) {
				if($value2->description == $value1){
					return TRUE;
				}
			}
		}
